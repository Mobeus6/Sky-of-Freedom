...
namespace SkyOfFreedom.Editor
{
    public class ImportWindow : EditorWindow
    {
        ...
        private const string ResearchFolder =
            "Assets/_Project/Data/ScriptableObjects/Research";
        ...
        private void OnGUI()
        {
                ...
                GUILayout.Height(40)))
            {
                string file =
                    EditorUtility.OpenFilePanel(
                        "Select Contracts CSV",
                        "",
                        "csv");

                if (!string.IsNullOrEmpty(file))
                {
                    ContractImporter.Import(
                        file,
                        ContractFolder);
                }
            }
