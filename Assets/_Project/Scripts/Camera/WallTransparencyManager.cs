...
public class WallTransparencyManager : MonoBehaviour
{
    ...
    [SerializeField] private float detectionWidth = 4f;

    [Tooltip("Висота області, в якій шукаються стіни.")]
    [SerializeField] private float detectionHeight = 4f;
    ...
    private void DetectWalls()
    {
        ...
        if (gameplayCamera == null || targetPoint == null)
            return;
        ...
        // Створюємо обертання BoxCast
        Quaternion rotation =
            Quaternion.LookRotation(direction);

        // Розмір області пошуку
        ...
        foreach (RaycastHit hit in hits)
        {
            ...
            WallTransparency wall =
                hit.collider.GetComponentInParent<WallTransparency>();
    ...
    {
        // Нові стіни робимо прозорими
        foreach (WallTransparency wall in detectedWalls)
        {
        ...
        // Стіни, які більше не перекривають огляд,
        // повертаємо до нормального стану
        wallsToRemove.Clear();

        foreach (WallTransparency wall in currentWalls)
        {
        ...
        foreach (WallTransparency wall in wallsToRemove)
        {
